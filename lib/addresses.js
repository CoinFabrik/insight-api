...
AddressController.prototype._getMultiTxsByAddress = function(groupByAddress, addrs, options, transformOptions, callback) {
  var self = this;
  if (!groupByAddress) {
    callback();
    return;
...
AddressController.prototype.multitxs = function(req, res, next) {
  ...
  self.node.getAddressHistory(req.addrs, options, function(err, result) {
    ...
    self.transformAddressHistoryForMultiTxs(result.items, transformOptions, function(err, items) {
      ...
      self._getMultiTxsByAddress(groupByAddress, req.addrs, options, transformOptions, function(err, byAddress) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
        var response = {
